...
public interface StatsRepository extends JpaRepository<Hit, Long> {
    ...
    @Query(" SELECT new ru.practicum.ewm.model.Stats(eh.app, eh.uri, COUNT(DISTINCT eh.ip)) " +
            "FROM Hit eh " +
    ...
    @Query(" SELECT new ru.practicum.ewm.model.Stats(eh.app, eh.uri, COUNT(eh.ip)) " +
            "FROM Hit eh " +
            "WHERE eh.timestamp BETWEEN ?1 AND ?2 " +
            "AND (eh.uri IN (?3) OR (?3) is NULL) " +
            "GROUP BY eh.app, eh.uri " +
